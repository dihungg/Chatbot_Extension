...
Nanobrowser is an open-source AI web automation Chrome extension that runs multi-agent systems locally in the browser. It's a free alternative to OpenAI Operator with support for multiple LLM providers (OpenAI, Anthropic, Gemini, Ollama, Llama, etc.).

## Principle Guardrails

All agents MUST follow the engineering principles defined in `PRINCIPLES.md` (SOLID, DRY, KISS, YAGNI, Separation of Concerns, Law of Demeter, etc.) while executing every task. Before delivering changes:

1. Cross-check new or modified code against the relevant sections of `PRINCIPLES.md` and adjust the implementation if it violates any guideline.  
2. Document trade-offs whenever a principle cannot be strictly satisfied (and explain why) inside the PR review summary or task notes.  
3. Prefer refactoring legacy code into compliance when touching it; never add new violations.  
4. If a requirement conflicts with a listed principle, escalate for clarification instead of silently deviating.  

These guardrails are mandatory—no code should ship without an explicit “principles pass.”
