      ...
      if (memoryItems.length > 0) {
        const memoryFormatted = memoryItems
          .map((p, idx) => {
            ...
            // --- [NEW LOGIC] EXTRACT SPECS FOR COMPARISON TABLE ---
            // Phần này giúp AI nhìn thấy ngay thông số kỹ thuật để so sánh A vs B
            let specHighlights = 'N/A';
            if (p.specs) {
              // @ts-ignore: Handle dynamic spec types based on schema
              if (p.specs.laptop) {
                // Ép kiểu 's' thành any để bypass lỗi thiếu property 'screen' trong type definition
                const s = p.specs.laptop as any; 
                specHighlights = `CPU: ${s.cpu_model || '?'} | GPU: ${s.gpu_model || '?'} | RAM: ${s.ram_gb}GB | Screen: ${s.screen || '?'}`;
              } else if (p.specs.phone) {
                 // @ts-ignore
                const s = p.specs.phone;
                specHighlights = `Chip: ${s.chipset || '?'} | Cam: ${s.camera_main_mp}MP | Battery: ${s.battery_mah}mAh`;
              // @ts-ignore
              } else if (p.specs.headphone) {
                 // @ts-ignore
                specHighlights = `ANC: ${p.specs.headphone.anc ? 'Yes' : 'No'} | Battery: ${p.specs.headphone.battery_hours}h`;
              }
            }
            // -------------------------------------------------------
...
[${idx + 1}]
PRODUCT_NAME: ${p.name}
PRICE: ${p.price_vnd?.toLocaleString('vi-VN')} VND
SOURCE: ${p.url}
KEY_SPECS: ${specHighlights} 
...
- Use SESSION MEMORY to compare specs between [1], [2]... found previously.
